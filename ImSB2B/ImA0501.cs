...
public class ImA0501
{
    ...
    public void Begin2(string path, string sKind0, ErrorInfoModel errorInfo, string identityKey)
    {
        ...
        using (SQL1 sqlAdapter = new SQL1())
        {
            List<EinvoiceA0501Temp> tempData = new List<EinvoiceA0501Temp>();
            var query = sqlAdapter.Kind1SelectTbl2("*", "FILE_TEMP", " EINVOICE_TP='A0501' and IDENT_KEY = '" + identityKey + "'", "", "");
            if (query != null)
            {
                var rows = query.Table.Rows;
                for (int i = 0; i < rows.Count; i++)
                {
                    tempData.Add(new EinvoiceA0501Temp()
                    {
            ...
            foreach (var data in tempData)
            {
                ...
                try
                {
                    ...
                    if (line.Trim() != "")
                    {
                        ...
                        if (charA.Length == 10)
                        {
                            if (sqlAdapter.Kind1SelectTbl3("A0501SN", "CancelInvoiceNumber='" + charA[0].ToString() + "'", "A0501") == 0)
                            {
                                ...
                                string insertMsg = sqlAdapter.InsertDataNonKey("A0501", hashData);
...
internal class EinvoiceA0501Temp
{
