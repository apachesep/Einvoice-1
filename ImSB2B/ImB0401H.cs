...
public class ImB0401H
{
    ...
    public void Begin2(string path, string sKind0, ErrorInfoModel errorInfo, string identityKey)
    {
        ...
        {
            List<EinvoiceB0401HTemp> tempData = new List<EinvoiceB0401HTemp>();
            var query = sqlAdapter.Kind1SelectTbl2("*", "FILE_TEMP", " EINVOICE_TP='B0401H' and IDENT_KEY = '" + identityKey + "'", "", "");
            if (query != null)
            {
                var rows = query.Table.Rows;
                for (int i = 0; i < rows.Count; i++)
                {
                    tempData.Add(new EinvoiceB0401HTemp()
                    {
            ...
            foreach (var data in tempData)
            {
                ...
                try
                {
                    ...
                    if (line.Trim() != "")
                    {
                        ...
                        if (charA.Length == 24)
                        {
                            if (sqlAdapter.Kind1SelectTbl3("B0401SN", "MAllowanceNumber='" + charA[0].ToString() + "'", "B0401H") == 0)
                            {
                                ...
                                string insertMsg = sqlAdapter.InsertDataNonKey("B0401H", hashData);
...
internal class EinvoiceB0401HTemp
{
