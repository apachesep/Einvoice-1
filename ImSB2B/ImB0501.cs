...
public class ImB0501
{
                        ...
                        if (line.Trim() != "")
                        {
                            ...
                            if (charA.Length == 9)
                            {
                                ...
                                using (NSysDB.NTSQL.SQL1 query = new NSysDB.NTSQL.SQL1())
                                {
                                    try
                                    {
                                        if (query.Kind1SelectTbl3("B0501SN", "CancelAllowanceNumber='" + charA[0].ToString() + "'", "B0501") == 0)
                                        {
                                            ...
                                            query.InsertDataNonKey("B0501", data);
                                            data = null;
                                        }
                                        else
    ...
    public void Begin2(string path, string sKind0, ErrorInfoModel errorInfo, string identityKey)
    {
        ...
        using (SQL1 sqlAdapter = new SQL1())
        {
            List<EinvoiceB0501Temp> tempData = new List<EinvoiceB0501Temp>();
            var query = sqlAdapter.Kind1SelectTbl2("*", "FILE_TEMP", " EINVOICE_TP='B0501' and IDENT_KEY = '" + identityKey + "'", "", "");
            if (query != null)
            {
                var rows = query.Table.Rows;
                for (int i = 0; i < rows.Count; i++)
                {
                    tempData.Add(new EinvoiceB0501Temp()
                    {
            ...
            foreach (var data in tempData)
            {
                ...
                try
                {
                    ...
                    if (line.Trim() != "")
                    {
                        ...
                        if (charA.Length == 9)
                        {
                            if (sqlAdapter.Kind1SelectTbl3("B0501SN", "CancelAllowanceNumber='" + charA[0].ToString() + "'", "B0501") == 0)
                            {
                                ...
                                string insertMsg = sqlAdapter.InsertDataNonKey("B0501", hashData);
...
internal class EinvoiceB0501Temp
{
