...
public class ImC0401H
{
    ...
    {
        /*
        12  [買方營業人統一編號須0000000000]          "                     , (counter + 1).ToString(), 11); }
        36  [ASalesAmount要>=0且不能有小數]            "                     , (counter + 1).ToString(), 11); }
        37  [AFreeTaxSalesAmount要>=0且不能有小數]     "            , (counter + 1).ToString(), 11); }
        38  [AZeroTaxSalesAmount要>=0且不能有小數]             "            , (counter + 1).ToString(), 11); }
        41  [ATaxAmount要>=0且不能有小數]          "                       , (counter + 1).ToString(), 11); }
        42  [ATotalAmount要>=0且不能有小數]        "                      , (counter + 1).ToString(), 11); }
        43  [ADiscountAmount要>=0且不能有小數]         "                , (counter + 1).ToString(), 11); }
        */
        int[] parserIndexAry = new int[] { 12, 36, 37, 38, 41, 42, 43 };
            ...
            switch (index)
            {
                case 0:
                    if (!data.Equals("0000000000"))
                        errorBuffer.Add(false, errorMsgTitle += "[買方營業人統一編號須0000000000]");
                    break;
    ...
    public void Begin2(string path, string sKind0, ErrorInfoModel errorInfo, string identityKey)
    {
        ...
        {
            List<EinvoiceC0401HTemp> tempData = new List<EinvoiceC0401HTemp>();
            var query = sqlAdapter.Kind1SelectTbl2("*", "FILE_TEMP", " EINVOICE_TP='C0401H' and IDENT_KEY = '" + identityKey + "'", "", "");
            ...
            if (query != null)
            {
                ...
                {
                    tempData.Add(new EinvoiceC0401HTemp()
                    {
            ...
            foreach (var data in tempData)
            {
                ...
                try
                {
                    ...
                    if (line.Trim() != "")
                    {
                        ...
                        if (charA.Length == 48)
                        {
                            if (sqlAdapter.Kind1SelectTbl3("C0401SN", "MInvoiceNumber='" + charA[0].ToString() + "'", "C0401H") == 0)
                            {
                                ...
                                string insertMsg = sqlAdapter.InsertDataNonKey("C0401H", hashData);
...
internal class EinvoiceC0401HTemp
{
